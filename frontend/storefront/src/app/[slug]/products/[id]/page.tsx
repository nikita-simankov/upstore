...
export default async function ProductPage({ params }: { params: { slug: string; id: string } }) {
  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-10">
      <a href={`/${params.slug}`} className="text-sm text-gray-500 hover:text-gray-900 block mb-8">← {store.name}</a>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
        ...
        <div className="flex flex-col">
          ...
          <p className="text-2xl font-bold text-gray-900 mt-3">{fmt(product.price)}</p>
          {product.description && <p className="text-gray-500 text-sm mt-4 leading-relaxed">{product.description}</p>}
          <div className="mt-auto pt-8">
            <AddToCartButton
              ...
              stock={product.stock}
            />
            {product.stock > 0 && <p className="text-xs text-gray-400 text-center mt-3">В наличии: {product.stock} шт.</p>}
          </div>
        </div>
      </div>
    </main>
